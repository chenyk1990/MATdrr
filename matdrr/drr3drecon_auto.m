function [ D1 ] = drr3drecon_auto(D,MASK,flow,fhigh,dt,N,K,Niter,eps,verb,mode,amode,a,eps2)
%  DRR3DRECON_AUTO: DRR for 3D seismic reconstruction (or F-XY domain damped multichannel singular spectrum analysis (DMSSA) for
%  simultaneous denoising and reconstruction) with automatically selected rank
%
%  IN   D:   	intput 3D data
...
%  Modified by Yangkang Chen, Oak Ridge National Lab, 2018
%
...
for k=ilow:ihigh
    ...
    for iter=1:Niter
        ...
        M=P_H(Sn_1,lx,ly);
        %         M=P_R(M,N);
        M=P_R(M,Nmin,Nmax,K,amode,eps2);
...
function [dout,H]=P_R(din,Nmin,Nmax,K,mode,eps)
% Rank reduction on the block Hankel matrix


%      [U,D,V]=svds(din,Nmax); % a little bit slower for small matrix
%      dout=U*D*V';
% %
[U,E,V]=svd(din);
...
if mode==1
    N=Nmax;
    for i=1:Nmax
        if((E(i,i)^2-E(i+1,i+1)^2)/(E(i,i)^2)) >=eps
            N=i;
            continue
        end
    end
...
for j=1:N
    E(j,j)=E(j,j)*(1-E(N+1,N+1)^K/(E(j,j)^K+0.000000000000001));
end

dout=U(:,1:N)*E(1:N,1:N)*(V(:,1:N)');
