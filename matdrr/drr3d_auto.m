function [ D1, H ] = drr3d_auto(D,flow,fhigh,dt,N,K,verb,mode,f,eps)
%  DRR3D_AUTO: 3D damped rank-reduction method with automatically chosen ranks (or F-XY domain damped multichannel singular spectrum analysis)
%
%  IN   D:   	 intput 3D data
...
%               2 (ratio)
%           f:  reference frequency for outputing singular values (energy)
%           eps: threshold (fixed in default)
%  
%  OUT  D1:  	output data
%       H: (if speicified), output the singular values.
% 
...
for k=ilow:ihigh
    ...
    if k~=ikf
    M=P_R(M,Nmin,Nmax,K,mode,eps);
    else
    [M,H]=P_R(M,Nmin,Nmax,K,mode,eps);
    end
